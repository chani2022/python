    ...
    def onNextFieldInStack(self, next_position):
        ...
        if line_edit:
            ...
            if not info["is_valid"]:
                ...
                return
            # if 'date_even' in name:
            #     if not DateTimeManager.isDateValid(content):
            #         QMessageBox.critical(self, "Erreur", "La date est invalid")
            #         return
            #     annee = content[-4:]
            #     if annee != self.annee_registre:
            ...
            #         return
            # if 'heure_' in name:
            #     if not DateTimeManager.isTimeValid(content):
            #         QMessageBox.critical(self, "Erreur", "La date est invalid")
            #         return
